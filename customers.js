        ...
        return __generator(this, function (_a) {
            return [2 /*return*/, new Promise(function (resolve, reject) {
                    database_1.pool.getConnection(function (err, connection) {
                        ...
                        var deleteQuery = 'DELETE FROM sylius_customer';
                        connection.query(deleteQuery, function (error, results, fields) {
                            if (error) {
                                reject(error);
                                return;
                            }
                            var resetQuery = 'ALTER TABLE sylius_customer AUTO_INCREMENT = 1';
                            connection.query(resetQuery, function (error, results, fields) {
                                if (error) {
                                    reject(error);
                                    return;
                                }
                                var customerInsertQueries = new Array();
                                var _loop_1 = function (i) {
                                    var customer = {
                                        first_name: faker_1.faker.name.firstName(),
                                        last_name: faker_1.faker.name.lastName(),
                                        email: faker_1.faker.internet.email(),
                                        gender: faker_1.faker.helpers.arrayElement(['M', 'Ms']),
                                        birthday: faker_1.faker.date.birthdate(),
                                        created_at: faker_1.faker.date.recent(),
                                        updated_at: faker_1.faker.date.recent()
                                    };
                                    var insertCustomerQuery = 'INSERT INTO sylius_customer SET ?';
                                    var insertCustomerPromise = new Promise(function (resolve, reject) {
                                        connection.query(insertCustomerQuery, customer, function (error, results, fields) {
                                            if (error) {
                                                reject(error);
                                                return;
                                            }
